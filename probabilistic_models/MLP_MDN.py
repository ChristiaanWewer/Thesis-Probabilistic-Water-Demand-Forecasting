...
naive_val = [torch.load(os.path.join(data_folder, 'val_naive_{}_{}_final2.pt'.format(ds, dma)), map_location=device) for dma in DMAs]
...
configuration = {
    ...
    'criterion':                           {'values': ['GMMLoss']},
    'probabilistic_method':                {'values': ['mixture_density_network']},
    'number_of_gaussians':                 {'values': [1,2,3,4]},
    'mdn_variance_regularization':         {'values': [0]},
    'mdn_weights_regularization':          {'values': [0]},
    'num_mdn_samples':                     {'values': [1000]},
    'quantiles':                           {'values': [[0.025,0.5, 0.975]]},
    'crit_nr_to_save':                     {'values': [1]}, # criterion we save 1=relative_pb (RQE in report)', 2=cov gap, 3=robust pinaw
...
def hyperparameter_optimization(config=None):
    with wandb.init(config=config, settings=wandb.Settings(_service_wait=300), job_type='fs'):
        ...
        for i, dma in enumerate(DMAs):
            ...
            y_pred_dma = FC.predict_mdn(x_test_dma, batch_size_pred=256, num_samples=1000, batch_size_samples=256)
